...
speaker_ids_main = os.path.join(base_path, "spk_ids_main_weighted_new_main_thsd_045")
...
def match_faces(known_faces, face_encoding, base_threshold=0.45):
...
# Function to log ambiguous matches with detailed metrics
def log_ambiguous_match(video_path, distances, avg_distance, dynamic_threshold):
    ambiguous_log_path = os.path.join(base_path, "ambiguous_matches.log")
    with open(ambiguous_log_path, "a") as log_file:
        log_file.write(f"Ambiguous match for {video_path}: Distances={distances}, Avg Distance={avg_distance}, Dynamic Threshold={dynamic_threshold}\n")
    ...
    for root, dirs, files in os.walk(path):
        for file in files:
            if file.endswith(".mp4"):
                ...
                face_encodings, confidence_scores, frame_qualities = extract_faces_from_video_retinaface(video_path, sample_rate=5)

                if not face_encodings:
                    print(f"No face detected in {video_path}. Skipping...")
                    continue

                # Use confidence scores and frame qualities as weights for the weighted average
                video_avg_encoding = weighted_average_encoding(face_encodings, confidence_scores, frame_qualities)

                identity_index, min_distance = match_faces(known_faces, video_avg_encoding, base_threshold=0.45)

                if identity_index == -1:
                    known_faces.append(video_avg_encoding)
                    identity_index = identity_count
                    identity_count += 1
                elif min_distance > 0.45:  # Ambiguity logging
                    avg_distance = np.mean([np.linalg.norm(known_face - video_avg_encoding) for known_face in known_faces])
                    dynamic_threshold = min(0.45, avg_distance * 0.8)
                    log_ambiguous_match(video_path, [min_distance], avg_distance, dynamic_threshold)
